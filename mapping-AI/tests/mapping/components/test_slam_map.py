...
def test_should_insert_value_as_map_coordinate():
    ...
    # assert np.all(slam_map.get_map() == np.flip(expected_map, 0))
...
def test_should_initialize_pre_built_map():
    ...
    # assert np.all(slam_map.slam_map == np.flip(expected_map, 0))
...
def test_should_return_sensory_array():
    ...
    # 20 x 20m grid
    actual_map = np.zeros((25 * METRE_2_PIX, 25 * METRE_2_PIX))
    actual_map[45:55, 50:60] = 1
    actual_map[450:600, 450:600] = 1

    slam_map = SlamMap(5, 5)
    slam_internal = SlamMap(20 * METRE_2_PIX, 20 * METRE_2_PIX)
