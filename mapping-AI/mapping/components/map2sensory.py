import numpy as np
import math
from mapping.components.constants import Action, Directions, MAX_RANGE, len2ind, cos, sin, DELTA_R
...
def sensor_array(current, direction, actual_map, fov):
    ...
    size_sensory_array = int(fov//Directions.DELTA_THETA)
    ...
    for scan in range(size_sensory_array):
        for i in np.arange(0, MAX_RANGE, DELTA_R):
            ...
            if xInd in (-1, size_sensory_array) or yInd in (-1, size_sensory_array):
                    sensory_array[scan] = np.sqrt((xInd/Directions.METRE_2_PIX)**2 + (yInd/Directions.METRE_2_PIX)**2)
            else:
